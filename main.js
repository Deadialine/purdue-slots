...
class SlotMachine {
  ...
  renderBetOptions() {
    ...
    this.config.betOptions.forEach((bet) => {
      ...
      option.innerHTML = `<input type="radio" name="bet" value="${bet}">${dollars(bet)}`;
      if (bet === this.currentBet) option.classList.add('active');
      option.addEventListener('click', () => this.setBet(bet, option));
